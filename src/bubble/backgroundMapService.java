    ...
    public void run() {
        ...
        while (true) {
            ...
            if(bottomLeft + bottomRight == -2) {
                // 계속 낙하
                // 발 아래가 허공(흰색)일 시 -> 점프 진행 중 or 낙하 중 -> 낙하 중이 아닐 때만 낙하 시작해야 함
                // player.iusup() == false = 점프 중 아님
                // player.isDown() == false = 낙하 중 아님
                if (player.isUp() == false && player.isDown() == false) {
                    player.down();
                }
            }else {
                // 발 아래가 바닥 / 층 -> 이라면 낙하 즉시 중단
                player.setDown(false); // while(down) --> false --> while문 종료 , Thread 종료
            }
